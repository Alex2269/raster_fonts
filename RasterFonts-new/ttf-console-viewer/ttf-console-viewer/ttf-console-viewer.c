...
const char* utf8_to_codepoint(const char* str, uint32_t* codepoint) {
    ...
    if (c < 0x80) {
        ...
        *codepoint = 0;  // Не підтримуємо 4-байтові символи
        return str + 1;
    }
}
...
/* Знаходимо максимально зайнятий по вертикалі рядок у бінарному бітмапі */
int find_bitmap_max_y(const uint8_t* bitmap, int bitmap_width, int bitmap_height) {
    int bytes_per_row = (bitmap_width + 7) / 8;
...
/* Вивід бітмапу у консоль текстом */
